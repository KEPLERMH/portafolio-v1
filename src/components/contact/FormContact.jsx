...
const FormContact = () => {
    ...
    return (
        <form aria-label='Formulario de contacto' onSubmit={handleSubmit(SendMessage)} className='flex flex-col gap-6'>
            ...
            <button type='submit' className='group flex justify-center items-center gap-2 py-4 rounded-full btn-custom transition duration-400 ease-in-out hover:shadow-lg
             hover:shadow-cyan-500/50 hover:scale-105'><span className='font-bold text-sm'>Enviar Mensaje</span><MessageSquareShare className='w-4 h-4 group-hover:translate-x-2 transition-transform duration-300 ease-out' /></button>
        </form>
    )
}
